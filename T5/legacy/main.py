...
if __name__ == '__main__':
    ...
    for batch in tqdm(test_loader):
        ...
        #                       outs.scores[i].item()] for i in range(len(outs))], key=lambda x: x[1], reverse=True)
        
        # prob = []
        # for idx in range(len(outs)):
            # print(outs.scores[idx])
            # print(tokenizer.decode(outs.sequences[idx], skip_special_tokens=True))
            # print(outs.sequences_scores[idx])
            # print('=================================')
        #     prob.append(outs.scores[idx][0])
        # print(tokenizer.decode(outs.sequences[np.argmax(prob)], skip_special_tokens=True))
        # print('@@', prob)
        ...
        for ids, s in zip(outs.sequences, outs.sequences_scores):
            if cnt_beams < 5:
                ...
                bug = [tokenizer.decode(idx, skip_special_tokens=True, 
                                clean_up_tokenization_spaces=False) for idx in pred]
                print(bug)
                print('-'*30)
                print(prob)
                print('='*30)
        ...
        source = [tokenizer.decode(ids, skip_special_tokens=True, 
            ...
            #                     clean_up_tokenization_spaces=False))
        # dec = tokenizer.decode(outs.sequences[np.argmax(prob)], skip_special_tokens=True)    # とりあえずうまくいかない！！！！！！！！！！！！
    ...
    # TODO: csv かtxt にログとして書き込めるようにする
    print('--- FINISH ---')
